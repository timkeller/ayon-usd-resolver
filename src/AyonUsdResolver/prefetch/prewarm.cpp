...
// Defined in resolverContext.cpp: the single process-wide resolver cache that
// every AyonUsdResolverContext shares. Seeding it warms resolution for the whole
// process, regardless of which context the stage open ends up using.
extern std::shared_ptr<ResolverContextCache> GlobalCache;
...
AyonUsdResolverPrewarmStage(const std::string &rootAssetPath) {
    if (rootAssetPath.empty() || !GlobalCache) {
        return;
    }
    if (GlobalCache->isCacheStatic()) {
    ...
    while (!layerFrontier.empty()) {
        ...
        if (!uriFrontier.empty()) {
            ++batchRounds;
            resolved = GlobalCache->batchWarm(uriFrontier);
